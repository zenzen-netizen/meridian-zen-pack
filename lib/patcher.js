...
export function applyPatch({ targetRoot, file, anchor, inject, marker, already = null, backupsDir, hashDbPath, nodeCheck = true }) {
  const abs = join(targetRoot, file);
  let src = readFileSync(abs, "utf8");
  const beginTag = `// >>> ${marker} >>>`;
  const endTag = `// <<< ${marker} <<<`;

  if (src.includes(beginTag) || (already && src.includes(already))) {
    return { status: "skipped-idempotent", file };
  }
...
export function replaceLine({ targetRoot, file, oldLine, newLine, already = null, backupsDir, hashDbPath, nodeCheck = true }) {
  const abs = join(targetRoot, file);
  const src = readFileSync(abs, "utf8");
  ...
  if (src.includes(newLine) || (already && src.includes(already))) {
    return { status: "skipped-idempotent", file };
  }
